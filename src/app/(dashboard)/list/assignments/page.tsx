...
import { FileText, User, LayoutGrid, Clock } from "lucide-react";
...
const AssignmentListPage = async ({
  ...
  // We filter Assignments via their related Lesson
  ...
  switch (role) {
    ...
    case "parent":
      ...
      break;
    default:
      // If role is unknown, restrict all data
      lessonQuery.id = -1; 
      break;
  }
  ...
  // Finalizing the main query object
      ...
      <tr key={item.id} className="border-b border-slate-100 last:border-0 text-sm hover:bg-slate-50/50 transition-all group">
        <td className="p-4">
          <div className="flex items-center gap-3">
            ...
            <span className="font-black text-slate-700 tracking-tight">{item.title || item.lesson.subject.name}</span>
          </div>
        </td>
        <td className="p-4 text-slate-500 font-bold">
          <span className="px-2 py-1 bg-slate-100 rounded-lg text-[10px] uppercase tracking-widest">
        ...
        <td className="hidden md:table-cell p-4 text-slate-500">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">{item.lesson.teacher.name} {item.lesson.teacher.surname}</span>
          </div>
        </td>
        <td className="hidden lg:table-cell p-4">
          <div className={`flex items-center gap-2 font-bold ${isOverdue ? 'text-rose-500' : 'text-slate-600'}`}>
            <Clock size={14} />
            <span>{new Intl.DateTimeFormat("en-GB", { day: '2-digit', month: 'short' }).format(item.dueDate)}</span>
          </div>
        </td>
  ...
  return (
    <div className="bg-white p-8 rounded-[2.5rem] flex-1 m-4 mt-0 shadow-sm border border-slate-100">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-6">
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tighter uppercase">Assignment Portal</h1>
