...
const ExamListPage = async ({
  ...
  // --- 2. ADMIN/TEACHER CLASS SELECTOR (CONSOLIDATED COHORTS) ---
  if (!classId && !search && role !== "student" && role !== "parent") {
    const rawClasses = await prisma.class.findMany({
      ...
      include: { level: true, supervisor: true, _count: { select: { students: true, exams: true } } },
      orderBy: { name: "asc" },
    });
    ...
    const consolidatedClasses = Object.values(consolidatedMap);
