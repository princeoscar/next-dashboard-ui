...
const MessageListClient = ({ initialMessages }: { initialMessages: MessagePreview[] }) => {
          ...
          >
            <div className="relative shrink-0">
              <div className="w-[50px] h-[50px] rounded-2xl overflow-hidden ring-2 ring-white shadow-sm bg-slate-100">
                <Image src={msg.avatar} alt={msg.name} width={50} height={50} className="object-cover w-full h-full" />
              </div>
              {msg.online && <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 border-4 border-white rounded-full" />}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-center mb-1">
                <span className={`font-bold truncate ${!msg.isRead ? "text-indigo-900" : "text-slate-700"}`}>{msg.name}</span>
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">{msg.time}</span>
              </div>
              <p className={`text-sm truncate ${!msg.isRead ? "text-slate-900 font-semibold" : "text-slate-500"}`}>{msg.lastMessage}</p>
            </div>
      ...
      {filteredMessages.length === 0 && (
        <div className="flex flex-col items-center justify-center py-32 space-y-6">
          ...
          <div className="text-center">
             ...
             <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.4em]">Try adjusting your search criteria</p>
          </div>
        </div>
      )}
    </div>
