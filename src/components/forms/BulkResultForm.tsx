...
export default function BulkResultForm({ students, action }: BulkResultFormProps) {
  // state will contain the success/error message from your server action
  const [state, formAction, isPending] = useActionState(action, null);

  return (
    <form action={formAction} className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 max-w-4xl relative">
      ...
      <div className="mt-10 flex justify-end">
        <button
          ...
          className={`flex items-center gap-2 px-10 py-4 rounded-3xl font-black text-[11px] uppercase tracking-widest transition-all ${
            isPending ? "bg-slate-400" : "bg-slate-900 text-white hover:bg-sky-600"
          }`}
        >
